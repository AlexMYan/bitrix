<?php
/**
 * Довйное свойство для пользовательских полей
 */
...
class CUserTypeIBlockElementList {
    ...
    function GetUserTypeDescription() {
        return array(
            ...
            'DESCRIPTION' => 'Привязка к элементам инф. блока в виде списка с описанием',
    ...
    function GetEditFormHTML($arUserField, $arHtmlControl) {
        ...
        $pieces = explode(":", $arHtmlControl['VALUE']);


        $iIBlockId = intval($arUserField['SETTINGS']['IBLOCK_ID']);
        $sReturn = '';
        $sReturn .= '<div>'.CUserTypeIBlockElementList::_getItemFieldHTML($pieces[0], $iIBlockId,
                $arHtmlControl['NAME']).'</div>';
        $sReturn .= '<input  name="'.$pieces[0].'_desc" type="text" value="'.$pieces[1].'">';
        return $sReturn;
    }
    ...
    function OnBeforeSave($arUserField, $value) {
        ...
        if(intval($value) > 0) {
            ...
            if(isset($_REQUEST[$value."_desc"])){
                $text=$value.":".$_REQUEST[$value."_desc"];
            }else{
                $text=$value.":".$_REQUEST["_desc"];
            }
            return $text;
